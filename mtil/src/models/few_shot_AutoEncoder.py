...
def few_shot_AutoEncoder(args):
    if args.eval_only == True:
        ...
        # train
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        # 加载clip模型，然后print一串text-transformer和image-transformer
        # 获得compose，用于预处理train和val数据
        ...
        print("batch nums:", len(few_shot_data_loader))
        # 使用adam优化器
        ...
        for iteration in tqdm(range(total_iterations + 1)):
            ...
            input_to_ae = feature_extractor(images)   # 64x256x13x13
            input_to_ae = input_to_ae.view(input_to_ae.size(0), -1)  # 64x43264
            ...
            outputs = model_autoencoder(input_to_ae)
            loss = encoder_criterion(outputs, input_to_ae)  # MSELoss
            loss.backward()  # 反向传播
            optimizer.step()  # 更新参数
            if iteration % 100 == 0:
                dataset_class = getattr(datasets, args.train_dataset)
