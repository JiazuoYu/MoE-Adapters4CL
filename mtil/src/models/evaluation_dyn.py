# --------------------------------------------------------
# Evaluation code of MoE-Adapters++ for a single task, based on CLIP
# --------------------------------------------------------
import clip
...
from clip.moe_layer import get_eval_zero_shot, get_val_task_id_visual
...
def dyn_zeroshot_eval(model, loader, zeroshot_weights, args):
    ...
    for i, data in enumerate(tqdm(loader, ncols=50)):
        ...
        if args.augmented_zero_shot and zero_shot_flag:
    ...
    for i in range(args.text_layer):
        if args.track_val_task_id[i]:
            if args.use_dyn_moe_layer_list_text[i]:
                ...
                if model.transformer.resblocks[i].reconginition_layer:
        ...
        if args.track_val_discrepancy[i]:
            if args.use_dyn_moe_layer_list_text[i] and model.transformer.resblocks[i].reconginition_layer:
                text_eval_discrepancy_list = model.transformer.resblocks[i].eval_discrepancy_list
    ...
    for i in range(args.vision_layer):
        if args.track_val_task_id[i]:
            if args.use_dyn_moe_layer_list_visual[i]:
                ...
                if model.visual.transformer.resblocks[i].reconginition_layer:
                    print(f"visual_zero_shot_rate: {visual_zero_shot_rate:.2f}")
                    print(f"visual_right_rate: {visual_right_rate:.2f}")
        if args.track_val_discrepancy[i]:
            if args.use_dyn_moe_layer_list_visual[i] and model.visual.transformer.resblocks[i].reconginition_layer:
                visual_eval_discrepancy_list = model.visual.transformer.resblocks[i].eval_discrepancy_list
...
def eval_loss_to_tensorboard(model, writer, idx, args,):
    for i in range(args.text_layer):
        if args.use_dyn_moe_layer_list_text[i]:
            text_mse_loss_list = [x for x in model.transformer.resblocks[i].eval_mse_list if x != 0]
    ...
    for i in range(args.vision_layer):
        if args.use_dyn_moe_layer_list_visual[i]:
            visual_mse_loss_list = [x for x in model.visual.transformer.resblocks[i].eval_mse_list if x != 0]
            # Recording Losses to TensorBoard
